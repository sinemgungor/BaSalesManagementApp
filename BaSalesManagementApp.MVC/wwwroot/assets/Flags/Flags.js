...
document.addEventListener("DOMContentLoaded", function () {
    // Initialize the international telephone input for the phone number
    var phoneInput = document.querySelector('#phoneInput');
    if (!phoneInput) return; // phoneInput yoksa devam etme

    var iti = intlTelInput(phoneInput, {
        separateDialCode: true,
        initialCountry: "tr",  // İsteğe bağlı, varsayılan ülke kodunu belirler
    });

    if (!iti) return; // iti oluşturulamazsa devam etme

    // Bayrak değiştirildiğinde alan kodunu güncelle
    phoneInput.addEventListener('countrychange', function () {
        const dialCode = iti.getSelectedCountryData().dialCode;
        phoneInput.value = "+" + dialCode + " ";
    });

    // Sayfa yüklendiğinde varsayılan ülke kodunu yerleştir
    const initialDialCode = iti.getSelectedCountryData().dialCode;
    phoneInput.value = "+" + initialDialCode + " ";

    // Telefon numarası giriş alanını dinle
    ...
    if (telInput) {
        ...
        console.error("Telefon numarası girişi bulunamadı.");
    }
});
...
function intlTelInputInit(elements) {
    ...
    var selectedCountries = (intlTelInput.preferredCountries && intlTelInput.preferredCountries.length)
        ? intlTelInput.preferredCountries.concat(intlTelInput.countries || [])
